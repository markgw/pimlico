...
class InputsCmd(PimlicoCLISubcommand):
    ...
    def run_command(self, pipeline, opts):
        ...
        try:
            ...
            print("Error: module '{}' does not exist".format(module_name), file=sys.stderr)
            sys.exit(1)
...
class MoveStoresCmd(PimlicoCLISubcommand):
    ...
    def run_command(self, pipeline, opts):
        ...
        if dest_store not in pipeline.store_names:
            print("No such store: {}".format(dest_store), file=sys.stderr)
            print("Available store names: {}".format(", ".join(pipeline.store_names)), file=sys.stderr)
        ...
        for module_name in opts.modules:
            ...
            if src_store is None:
                print("Cannot move data for '{}' as it wasn't found in any store".format(module_name), file=sys.stderr)
                continue
            elif src_store == dest_store:
                print("Skipping '{}' as it's alreay in store '{}'".format(module_name, dest_store), file=sys.stderr)
                continue
