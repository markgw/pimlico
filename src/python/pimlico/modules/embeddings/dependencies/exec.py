...
class ModuleExecutor(DocumentMapModuleExecutor):
    ...
    def process_document(self, archive, filename, doc):
        ...
        for sentence in doc:
            word_field = "lemma" if self.info.options["lemma"] else "word"
            ...
            for token in sentence:
                ...
                if token["deprel"] in self.info.options["skip_types"]:
                    continue
                ...
                if not self.info.filter_pos \
                        or pos in self.info.include_tags \
                        or any(pos.startswith(t) for t in self.info.include_tag_prefixes):
                ...
                if not self.info.filter_pos \
                        or head_pos in self.info.include_tags \
                        or any(head_pos.startswith(t) for t in self.info.include_tag_prefixes):
