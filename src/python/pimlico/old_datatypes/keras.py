...
from pimlico.old_datatypes.base import PimlicoDatatypeWriter, PimlicoDatatype
...
class KerasModelBuilderClassWriter(PimlicoDatatypeWriter):
    """
    ...
    def __init__(self, base_dir, build_params, builder_class_path, **kwargs):
        super(KerasModelBuilderClassWriter, self).__init__(base_dir, **kwargs)
        self.require_tasks("architecture", "weights")
        self.weights_filename = os.path.join(self.data_dir, "weights.hdf5")
    ...
    def __enter__(self):
        super(KerasModelBuilderClassWriter, self).__enter__()
